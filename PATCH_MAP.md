...
|---|---|---|
| `src/rendezvous_server.rs:522-532`, `handle_tcp` → `RequestRelay` | forwarded to the peer with **no authorization and no licence-key check** — a second, independent way into a controlled device that never passes `handle_punch_hole_request`. Pinned by `request_relay_still_bypasses_the_chokepoint` in S7 | **T3.3b** |

...
| S41 | applied 2026-09-15 (T5.6) | `tests/t56_failure_injection.rs` (new), `tests/harness/fault.rs` (new), `tests/harness/world.rs` (`fault_injection`), `tests/harness/mod.rs` (`register` docstring) | failure injection on the api path: the api killed outright, a fault proxy between hbbs and the api for the modes the api cannot produce, and ten of them swept for a single allow — plus the three separate claims behind "does not wedge" | D1 is the whole login layer, and an outage is the one condition under which a mistake here costs everything rather than one connection. Two of the three faults the task names — slow, garbage — cannot be produced by `apps/api` without shipping a fault mode in the product, so they go in the wire; and the proxy carries **only** hbbs, so the console stays readable while hbbs can get no answer at all | Isolated | low — test-only || S42 | applied 2026-09-15 (T5.7) | `tests/t57_bypass.rs` (new), `tests/harness/api.rs` (`set_user_disabled`, `try_login`), `tests/harness/mod.rs` (`online_request`, `online_bit`), `tests/harness/relay.rs` (stale name in a docstring) | bypass attempts on every door: empty, forged, expired and misplaced tokens, a disabled account, a client that skips login on both gates, and five `hbbr` cases including a keyless relay and a third party holding a live session's uuid | the task board asks for `hbbr`'s safety to be **proved**, and a claim that a component authorizes nothing is only worth what a test of the real binary says. The token cases are cheap; the two enumeration oracles they turned up were not otherwise written down anywhere | Isolated | low — test-only || S43 | applied 2026-09-15 (T5.8) | `tests/t58_latency.rs` (new), `tests/harness/world.rs` (`authorization`) | the connect path timed with authorization off, on with the cache off, and on as it ships — plus the authorize step alone, read from hbbs's own decision counter, against the 300 ms timeout that ships | T3.2 chose a timeout and a cache TTL from reasoning, and nothing had measured either. The baseline has to be produced in the same run on the same host, because a figure from T0.5 on another day is not a number this one can be subtracted from | Isolated | low — test-only || S44 | applied 2026-09-15 (T5.9) | `tests/t59_network_edges.rs` (new), `tests/harness/peer.rs` (`punch_alone`), `tests/harness/api.rs` (`logout`) | the connect path when the network or the token moves: a retried handshake as one decision, a reconnect past the cache as a fresh one, a device that comes back on a new port, and a token that expires, is revoked, or is refreshed under a live session | each of these has a "must" and a "must not" that are easy to get backwards, and the "must not" half is the expensive one — a session that dies at a token boundary is indistinguishable from a network fault to whoever is on the call, and the sweep that would do it runs against every live session in the fleet | Isolated | low — test-only || S45 | applied 2026-09-15 (T5.10) | `tests/t510_relay_matrix.rs` (new), `tests/harness/peer.rs` (`register_from`) | the access matrix, revocation, fail-closed and the bypass rows re-run with `ALWAYS_USE_RELAY=Y` — plus both halves of what the flag actually does, pinned | the relay is a separate gate with a separate call site, and "expected not to differ" is precisely the assumption that left `handle_request_relay` unauthorized until T3.3b. Re-running is cheap; finding out in the field is not | Isolated | low — test-only |

### Notes on the `S` rows

**S45 pins that `ALWAYS_USE_RELAY=Y` does nothing on one IP, and it is the
first test in the file for a reason.** The override rewrites `nat_type` only,
while `same_intranet` is computed independently and selects the
`FetchLocalAddr` branch, which carries no `nat_type` at all
(`rendezvous_server.rs:760-763`). Every peer in this harness is on 127.0.0.1, so
simply switching the flag on and re-running the earlier suites would have
produced tests that looked like they forced the relay and did not. The rest of
the file therefore drives the relay gate explicitly, which is the only way to be
certain which of the two gates answered. `register_from` exists to show the other
half — reach the device on the host's LAN address and the flag bites, which is
the shape a real deployment with `hbbs` on a public host would see only when its
clients are *not* all behind one NAT.

**S44 pins that a bare `RegisterPk` does not move a device.** The test is
deliberately written as the *negative* — re-register the key from a new socket,
watch the brokerage go to the old address — because this is the second time
`update_pk`'s skipped write has cost a day (T5.6 was the first, on
`last_reg_time`). It is upstream's behaviour and harmless in the field, since a
real client heartbeats every 15 s and re-registers its key almost never. The test
exists so that a change to the `changed` test at `rendezvous_server.rs:603` is
deliberate, and so that the next person to write a harness device does not spend
the afternoon on it.

**S44's three "must not" tests each spend nine seconds on purpose.** A token
expiring, a sign-out, and a refresh all have to leave a running session alone,
and the thing that would break that — `enforceLiveSessionAccess` — runs
unattended every three seconds against every live session there is. Asserting
that nothing happened needs several real beats, not one; a shorter patience would
pass against a sweep that simply had not run yet.

**S43's baseline is an `hbbs` with no authorization configured, not a stashed
number.** `WorldBuilder::authorization(false)` leaves `AUTH_API_URL` unset, which
is what turns enforcement off (`auth.rs:106`), so the baseline world runs
upstream's connect path with our two call sites reduced to one branch each — same
host, same run, same fixtures, same api serving the console. Subtracting two
measurements taken that way is meaningful; subtracting one taken on another
machine on another day is not.

**S43's assertions are loose and the printed table is the deliverable.** A tight
bound on a timing test run alongside other worlds fails for reasons that have
nothing to do with this code, and a perf test people learn to ignore is worse
than none. What is asserted is the shape: a cached decision must not be
measurable next to a TCP round trip, an uncached one must stay inside a couple of
indexed lookups, and the worst authorize step must use less than half the shipped
`AUTH_TIMEOUT_MS`. The numbers themselves are in docs/CONTEXT.md §"How the hook
is configured".

**S42 pins two oracles it does not close, and says why in the test.** An
unauthenticated stranger can tell a real device id from a fake one (the
peer-exists check runs before authorization, deliberately — the alternative is
one api request per guess), and `OnlineRequest` on `port - 1` answers anybody
with an online bitmap (the client's peer list depends on it). Neither gets a
stranger a connection. Both are recorded in docs/CONTEXT.md §"What a stranger
can still learn" and belong in the deployment notes rather than in a patch.

**S42's `hbbr_relays_for_any_two_peers_that_present_the_same_uuid` is meant to
be uncomfortable.** Two peers with no accounts, never introduced, relaying bytes
is the honest statement of what "hbbr authorizes nothing" means — and it is the
test that would fail if someone ever decided hbbr could be given a security job.
The three tests around it are the reason that is survivable: an established
session cannot be joined, a guess is answered with silence rather than a hint,
and a wrong licence key is closed without a word.
...
in the peer table, the per-device in-flight claim, and the global
`ENROL_RATE_PER_MINUTE` bucket — and a merge can neuter any of them without
failing to compile. Counting `/api/internal/enrolled` in the api's own request
log is the only measure that notices.

**S39 pins a property "single use" does not describe.** A capability is
reusable for `AUTH_CACHE_TTL_MS` against the same `(token, to_id, conn_type)`,
because the decision cache sits in front of the break-glass verifier so that a
client's retries do not burn it (`auth.rs:805-825`) — then it is refused as a
replay. The test asserts both halves, so a merge that moves the cache behind the
verifier fails on the first half rather than silently making the emergency path
fail for slow peers. Recorded in docs/CONTEXT.md, which did not say so.

**S38 measures decision D2 rather than asserting it.** Revocation costs
**one heartbeat** — 3.009 s direct, 3.011 s relayed, against a 3 s `TIME_CONN`
floor — because `enforceLiveSessionAccess` and `drainForDevice` run in the same
heartbeat request, in that order. A merge that reorders them, or that moves the
sweep off the heartbeat, turns one beat into two or into never; the printed
...
| `Enrolment`'s pk check | `apps/api` records a changed public key and does **not** refuse on it, because which key pair `rustdesk --deploy` reads on an installed host is not settled — T3.5.1 could not run the CLI (it needs `/Applications` + root). The strict reading of T3.5.2 is one line away in `services/enrolment.ts`. **T5.5 ran and could not settle it either**: it tests the protocol, not an installed client, and the blocker is unchanged. The recording half is at least visible now — `pkChangedAt` reaches the console, which it did not before T5.5 | needs a person on an **installed** host with root |
